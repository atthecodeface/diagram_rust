    ...
    E: HmlError<Position = P>,
{
    ...
    //fi bad_element_name
    pub(crate) fn bad_element_name(
        namespace_stack: &NamespaceStack<'_>,
        span: &HmlSpan<P>,
        tag: &HmlTag,
    ) -> Self {
    ...
    pub(crate) fn bad_element_name_expected(
        namespace_stack: &NamespaceStack<'_>,
        span: &HmlSpan<P>,
        tag: &HmlTag,
        name_ids: &NameIds,
        ...
        if expected.len() == 1 {
            let _ = write!(
                &mut expectation,
                ", expected '{}'",
                name_ids.str_of_name(&expected[0])
            );
        } else {
    ...
    pub(crate) fn bad_attribute_name(
        namespace_stack: &NamespaceStack<'_>,
        span: &HmlSpan<P>,
        attr: &HmlAttribute,
    ) -> Self {
    ...
    //fi value_result
    pub(crate) fn value_result<V, Err: std::fmt::Display>(
        span: &HmlSpan<P>,
        result: Result<V, Err>,
    ) -> Result<V, Self> {
        match result {
    ...
    //fi element_result
    pub(crate) fn element_result<V, Err: std::fmt::Display>(
        span: &HmlSpan<P>,
        result: Result<V, Err>,
    ) -> Result<V, Self> {
        match result {
    ...
    pub fn write_without_span(&self, f: &mut dyn std::fmt::Write) -> std::fmt::Result {
        match self {
            Self::EndOfStream => write!(
                f,
                "Unexpected end of XML event stream - bug in event source"
            ),
