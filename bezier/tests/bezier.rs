...
fn test_quadratic() {
    ...
    fn test_cubic() {
        ...
        let x = Bezier::arc((90.0f64).to_radians(), 1., &p0, &Point::from_array([1.,0.]), &Point::from_array([0.,1.]), 0.);
        println!("{}",x);
    ...
    fn test_arc() {
        ...
        let x = Bezier::arc((90.0f64).to_radians(), 1., &origin, &x_axis, &y_axis, 0.);
        println!("arc((90.).to_radians(), 1., &origin, &x_axis, &y_axis, 0.) : {}",x);
        bezier_eq(&x, vec![[1.,0.], [1.,magic], [magic,1.], [0.,1.]]);

        let x = Bezier::arc((90.0f64).to_radians(), 1., &origin, &x_axis, &y_axis, (-90.0f64).to_radians());
