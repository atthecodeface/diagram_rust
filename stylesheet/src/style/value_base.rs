...
mod test_res {
    ...
    fn test_extract_vec<R: FromStr + Debug + PartialEq>(
        rex: &Regex,
        s: &str,
        max_len: usize,
        expected: Vec<R>,
...
pub enum BaseValue {
    ...
    ///
    /// This first element is the 'separator'; the bool allows "" to be added to the vector
...
impl BaseValue {
    ...
    pub fn floats(n: usize) -> Self {
        Self::Floats(n, Vec::new())
    }

    //fp float_array
    pub fn float_array() -> Self {
        Self::FloatArray(Vec::new())
    }
...
impl TypeValue for BaseValue {
    ...
    fn from_string<'a>(&'a mut self, s: &str) -> Result<&'a mut Self, ValueError> {
        match self {
            ...
            Self::Rgb(ref mut f) => match color::of_string(s) {
                Some(rgb) => {
                    color::as_floats(rgb, f);
                }
                None => {
